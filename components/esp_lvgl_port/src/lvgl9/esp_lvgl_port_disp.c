...
static lv_display_t *lvgl_port_add_disp_priv(const lvgl_port_display_cfg_t *disp_cfg, const lvgl_port_disp_priv_cfg_t *priv_cfg)
{
    ...
    if (priv_cfg && priv_cfg->avoid_tearing) {
        ...
        ESP_LOGI("MEM", "LVGL buffer size4: %lu bytes", buffer_size * sizeof(lv_color_t));
        // heap_caps_defrag(MALLOC_CAP_DEFAULT);  // Дефрагментируем RAM
        ESP_LOGI("MEM", "Largest free block: %d", heap_caps_get_largest_free_block(MALLOC_CAP_8BIT));
